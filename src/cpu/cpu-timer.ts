...
export class CpuTimer {
  ...
  public tick(): void {
    ...
    const control = this.addressBus[0xFF07].byte;
  ...
  private incrementDivider(): void {
    this.addressBus[0xFF04].byte = (this.addressBus[0xFF04].byte + 1) & 0xFF;
  }

  private incrementCounter(): void {
    let newValue = (this.addressBus[0xFF05].byte + 1) & 0xFF;

    // Triggers interrupt and reset to modulo in case of an overflow
    if (newValue === 0) {
      this.addressBus[0xFF0F].byte = this.addressBus[0xFF0F].byte | 0b100;
      newValue = this.addressBus[0xFF06].byte;
    }

    this.addressBus[0xFF05].byte = newValue;
  }
}
